...
contract BNBPosiVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function canCompound() public view returns (bool, uint256) {
        bool p1 = posiStakingManager.canHarvest(POSI_BNB_PID, address(this));
        bool p2 = posiStakingManager.canHarvest(POSI_SINGLE_PID, address(this));
        return (p1 || p2, p2 ? POSI_SINGLE_PID : POSI_BNB_PID);
    }

    function nearestCompoundingTime() public view returns (uint256) {
        (,,,uint256 pool1CompoundingTime) = posiStakingManager.userInfo(POSI_BNB_PID, address(this));
...
//        busd.approve(address(router), MAX_INT);
    ...
    // deposit BNB
    function deposit() external payable updateReward(msg.sender) nonReentrant {
        uint256 amount = msg.value;
        IUniswapV2Pair pair = getSwappingPair();
        (uint256 res0, uint256 res1, ) = pair.getReserves();
        uint256 amountToSwap = calculateSwapInAmount(res1, amount);
        (uint256[] memory amounts) = router.getAmountsOut(amountToSwap, getWbnbPosiRoute());
        ...
        uint256 amountLeft = amount.sub(amountToSwap);
        {
            ...
            //stake in farms
            posiStakingManager.deposit(POSI_BNB_PID, liquidityAmount, address(this));
            //set state
    ...
    function withdraw(uint256 amount) external updateReward(msg.sender) nonReentrant {
        ...
        (uint256[] memory amounts)= router.getAmountsIn(amount, getPosiBnbRoute());
        ...
        if(reserveAmount >= amountNeededInPosi){
            ...
            //swap for amount
            router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, getPosiBnbRoute(), address(this), 0);
        }else{
            ...
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountA, 0, getPosiBnbRoute(), address(this), block.timestamp);
    ...
    function getWbnbPosiRoute() private view returns(address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(wbnb);
        paths[1] = address(posi);
    }
    function getPosiBnbRoute() private view returns(address[] memory paths) {
        paths = new address[](2);
